...
test.describe('14.1 · the manufacturer calendar', () => {
  ...
  test('a scheduled survey appears on its Istanbul day and links to the request', async ({
    ...
    /*
     * `ORDER BY` so repeat runs pick the SAME request. Without it `LIMIT 1` returned an
     * arbitrary row, the run planted an appointment on a different request than the last
     * one, and the assertion below matched the older chip — green on a fresh database and
     * red on the second run (found in 14.4, by running the suite twice).
     */
    const [request] = await pgQuery<{ id: string }>(
      `SELECT "id" FROM "OfferRequest" WHERE "companyId" = $1 ORDER BY "id" ASC LIMIT 1`,
      [id],
    )
    ...
    const plantedId = `apt_e2e_calendar_${request?.id}`

    /*
     * Sweep before planting, and inside the test rather than in `afterAll` — task 14.4.
     *
     * Two reasons, both learned by running the suite twice. **Leftovers pile into one cell:**
     * every run plants at the same instant on the same day, so three of them pushed this
     * run's chip behind the grid's "+1 more" and turned the assertion into a flake. And
     * **`afterAll` runs per worker**: Playwright spreads one file's tests across workers, so
     * a sibling worker's hook can delete the row this test just planted, mid-run.
     *
     * By prefix, which is safe here and would not be in `phase2-gate.spec.ts`: this string
     * is generated by this file and by nothing else, whereas a display name can be typed by
     * a person debugging.
     */
    await pgQuery(`DELETE FROM "Appointment" WHERE "id" LIKE 'apt_e2e_calendar_%'`, [])
    ...
    /*
     * Located by BOTH the request it belongs to and its kind. `.first()` on the kind alone
     * asserted "some survey chip exists and happens to be this one", which is a weaker claim
     * and the one that broke on a repeat run.
     *
     * `:visible` because the board renders the same event twice — the grid above `lg`, the
     * agenda below it — and only one of them is on screen at any width. Without it the
     * locator can settle on the hidden copy, which is a true statement about the DOM and a
     * false one about what the manufacturer sees.
     */
    const selector = `a[href$="/talepler/${request?.id}"][title^="Keşif randevusu"]`
    await expect(page.locator(`${selector}:visible`)).toBeVisible()
    // Both renderings carry it, which is the property that keeps them from disagreeing.
    await expect(page.locator(selector)).toHaveCount(2)

    // The seeded data goes back as it was: this spec writes one row and takes it away.
    await pgQuery(`DELETE FROM "Appointment" WHERE "id" = $1`, [plantedId])
  })
