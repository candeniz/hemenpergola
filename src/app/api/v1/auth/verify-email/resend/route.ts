...
/**
 * `POST /api/v1/auth/verify-email/resend` — issue a new verification link.
 *
 ...
 *
 * Issuing invalidates the outstanding one, so three clicks leave one live link, not three.
 *
...
export async function POST(request: Request): Promise<Response> {
  ...
  const parsed = resendEmailVerificationSchema.safeParse(await request.json().catch(() => null))
  if (!parsed.success) return respond(err(validation(parsed.error.issues)))

  const actor = await resolveActor(request)
  return respond(await resendEmailVerification(actor, parsed.data))
}
