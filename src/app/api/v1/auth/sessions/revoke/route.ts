...
/**
 * `POST /api/v1/auth/sessions/revoke` — end one session, or every other one.
 *
...
export async function POST(request: Request): Promise<Response> {
  const [{ revokeSessionSchema }, { revokeSession }, { resolveActor }, { err, validation }] =
    await Promise.all([
  ...
  return respond(await revokeSession(actor, parsed.data))
}
