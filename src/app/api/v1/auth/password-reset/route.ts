...
/**
 * `POST /api/v1/auth/password-reset` — ask for a reset link.
 *
 ...
 *
 * Always answers `{ sent: true }`. A 404 for an unknown address would be an account-enumeration oracle on a page nobody watches.
 *
...
export async function POST(request: Request): Promise<Response> {
  ...
  return respond(await requestPasswordReset(actor, parsed.data))
}
