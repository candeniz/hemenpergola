...
/**
 * `POST /api/v1/auth/phone/confirm` — check the OTP.
 *
 ...
 *
 * Five attempts, then the code is dead — six digits is only a credential because of the cap.
 *
...
export async function POST(request: Request): Promise<Response> {
  ...
  const parsed = confirmPhoneVerificationSchema.safeParse(await request.json().catch(() => null))
  if (!parsed.success) return respond(err(validation(parsed.error.issues)))

  const actor = await resolveActor(request)
  return respond(await confirmPhoneVerification(actor, parsed.data))
}
