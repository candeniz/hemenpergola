...
/**
 * `POST /api/v1/admin/catalog/options/deactivate` — hide from new projects.
 *
 ...
 *
 * `10` §Admin authoring: existing `ProjectAttributeValue` rows keep referencing it and still render. This is the action the screen offers by default.
 *
...
export async function POST(request: Request): Promise<Response> {
  const [{ deactivateOptionSchema }, { deactivateOption }, { resolveActor }, { err, validation }] =
    await Promise.all([
  ...
  const parsed = deactivateOptionSchema.safeParse(body)
  if (!parsed.success) return respond(err(validation(parsed.error.issues)))

  const actor = await resolveActor(request)
  return respond(await deactivateOption(actor, parsed.data))
}
