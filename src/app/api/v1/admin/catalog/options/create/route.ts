...
/**
 * `POST /api/v1/admin/catalog/options/create` — a choice on a SELECT attribute.
 *
...
export async function POST(request: Request): Promise<Response> {
  const [{ createOptionSchema }, { createOption }, { resolveActor }, { err, validation }] =
    await Promise.all([
  ...
  return respond(await createOption(actor, parsed.data))
}
