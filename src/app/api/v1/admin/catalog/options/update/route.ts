...
/**
 * `POST /api/v1/admin/catalog/options/update` — relabel or reorder.
 *
...
export async function POST(request: Request): Promise<Response> {
  const [{ updateOptionSchema }, { updateOption }, { resolveActor }, { err, validation }] =
    await Promise.all([
  ...
  return respond(await updateOption(actor, parsed.data))
}
