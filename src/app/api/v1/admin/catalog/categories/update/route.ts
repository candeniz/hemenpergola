...
/**
 * `POST /api/v1/admin/catalog/categories/update` — edit, reorder or deactivate.
 *
...
export async function POST(request: Request): Promise<Response> {
  const [{ updateCategorySchema }, { updateCategory }, { resolveActor }, { err, validation }] =
    await Promise.all([
  ...
  return respond(await updateCategory(actor, parsed.data))
}
