...
/**
 * `GET /api/v1/admin/catalog/categories` — the category tree.
 *
...
export async function GET(request: Request): Promise<Response> {
  const [{ listCategoriesSchema }, { listCategories }, { resolveActor }, { err, validation }] =
    await Promise.all([
  ...
  const parsed = listCategoriesSchema.safeParse({
    includeInactive: url.searchParams.get('includeInactive') === 'true',
  })
  if (!parsed.success) return respond(err(validation(parsed.error.issues)))

  const actor = await resolveActor(request)
  return respond(await listCategories(actor, parsed.data))
}
