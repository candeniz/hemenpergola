...
/**
 * `POST /api/v1/admin/catalog/attributes/delete` — refused while anything references its options.
 *
...
export async function POST(request: Request): Promise<Response> {
  const [{ deleteAttributeSchema }, { deleteAttribute }, { resolveActor }, { err, validation }] =
    await Promise.all([
  ...
  return respond(await deleteAttribute(actor, parsed.data))
}
