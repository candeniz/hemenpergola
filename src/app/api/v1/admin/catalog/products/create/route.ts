...
/**
 * `POST /api/v1/admin/catalog/products/create` — a new product. `CAT-03`: no deployment.
 *
...
export async function POST(request: Request): Promise<Response> {
  const [{ createProductSchema }, { createProduct }, { resolveActor }, { err, validation }] =
    await Promise.all([
  ...
  return respond(await createProduct(actor, parsed.data))
}
