...
/**
 * `POST /api/v1/admin/catalog/products/update` — edit, reorder or deactivate.
 *
...
export async function POST(request: Request): Promise<Response> {
  const [{ updateProductSchema }, { updateProduct }, { resolveActor }, { err, validation }] =
    await Promise.all([
  ...
  return respond(await updateProduct(actor, parsed.data))
}
