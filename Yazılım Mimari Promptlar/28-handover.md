...
*CI has run, and the first run found what "never run" predicted.* The first push came up red
in the `static` stage: pnpm 11 blocks postinstall scripts (`pnpm-workspace.yaml`
`allowBuilds`), so no job had a generated Prisma client and typecheck failed on missing
types — invisible locally, where the client had been generated by hand. Two more gaps were
fixed before the runner even reached them: the e2e job had no database at all (`ci-local.mjs`
ran on a machine where the compose stack was already up, so nothing noticed), and the
"workflow contains no deploy stage" unit test tripped over the string `prisma migrate
deploy` once the e2e job gained migrations. Every job now generates the client after
install, and the e2e job starts the compose stack, migrates and seeds before Playwright.
